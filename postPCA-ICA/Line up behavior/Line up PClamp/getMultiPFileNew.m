function planeItRanges = getMultiPFileNew(nFrames,nPlanes,nExtraPlanes,...
    pPath,mouseName,acqDate,tiffBase)
...
while ~gotAllPFiles
    ...
    if ~multiFiles || fileInd >= nFiles
        if ~firstGo
            %get pFile
            origDir = cd(fullfile(pPath,mouseName,acqDate)); %change to pClamp dir
            ...
            origDir = cd(fullfile(pPath,mouseName,acqDate)); %change to pClamp dir
            pDir = fullfile(pPath,mouseName,acqDate);
            pName = dir2cell(pDir);
            pName = pName(3:end);
            firstGo = false;
        end
