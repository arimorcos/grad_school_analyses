function autoPostAcq2PRed(folderPath,ask,vCellFile)
...
for planeInd = 1:nPlanes %for each plane
    ...
    extractedTracePath = sprintf('%s%s%s_extractedTraces_red.mat',folderPath,filesep,acqName);
    addMovieSizes(acq);
    if ~exist(extractedTracePath,'file')
        [dFF, ~, ~, ~] = extractROIsTIFF(acq,1,[],1,2);
        save(extractedTracePath,'dFF');
    else
        load(extractedTracePath);
    end
    if isfield(acq.roiInfo.slice(1).roi, 'group')
        isRoiSelected = ismember([acq.roiInfo.slice(1).roi.group], 1);
    else 
        isRoiSelected = ismember(acq.roiInfo.slice(1).grouping, 1);
    end
    roi = acq.roiInfo.slice(1).roi(isRoiSelected);
    fprintf('Complete\n');
    ...
    if isfield(acq.roiInfo.slice(1).roi, 'group')
        ...
        dataCell = copyFDataToCell(dataCell,trialIDs,planeInd,dataFrames,dFF,...
            acq.roiInfo.slice(1).grouping);
    end
    
    for cell = 1:length(dataCell)
        dataCell{cell}.imaging.mode = 'RED';
    end
end

%save dataCell
% newName = 
[path,name] = fileparts(vCellFile);
newName = fullfile(path,sprintf('%s_red.mat',name));
fprintf('Saving dataCell...');
save(newName,'dataCell');
fprintf('Complete\n');
