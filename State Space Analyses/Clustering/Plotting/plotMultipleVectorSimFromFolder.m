function handles = plotMultipleVectorSimWithShuffleFromFolder(folder,fileStr,correlation)
...
barData = cat(2,meanDiag,meanOffDiag);
barError = cat(2,semDiag,semOffDiag);
...
[~,sig] = ttest2(plotDataDiag(:),plotDataOffDiag(:));
...
bracketX = [errH(1).XData(1) errH(2).XData(1)];
% bracketX = 1:3;
% groups = {bracketX([1 2]),bracketX([1 3]),bracketX([2 3])};
groups = {bracketX([1 2])};
...
%label 
axH.XTickLabel = {'Intra-cluster','Inter-cluster'};
