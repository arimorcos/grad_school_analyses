function plotDeltaSegVectors(dataCell,whichPlots,binPoints)
%plotDeltaSegVectors.m Plots the delta segment vector start distance
%correlation
%
%INPUTS
...
for plotInd = 1:nPlots
    ...
    if binPoints
        ...
        for binInd = 1:nBins
            ...
            meanEnd(binInd) = mean(deltaVec{whichPlots(plotInd)}(matchInd));
            semEnd(binInd) = calcSEM(deltaVec{whichPlots(plotInd)}(matchInd)');
        end
        ...
        scatH = scatter(deltaSegStart{whichPlots(plotInd)},deltaVec{whichPlots(plotInd)});
        
        allPoints = cat(2,deltaSegStart{whichPlots(plotInd)},deltaVec{whichPlots(plotInd)});
        lims = [min(allPoints) max(allPoints)];
    end
    axis(axH,'square');
    ...
    [corr,pVal] = corrcoef(deltaSegStart{whichPlots(plotInd)},deltaVec{whichPlots(plotInd)});
    ...
    %fit lines and plot
    fitCoeff = robustfit(deltaSegStart{plotInd},deltaVec{plotInd});
