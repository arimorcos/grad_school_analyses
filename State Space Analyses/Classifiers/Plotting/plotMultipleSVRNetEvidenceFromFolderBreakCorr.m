...
allClassOutBreakCorr = cell(length(matchFiles),1);
for fileInd = 1:length(matchFiles)
    currFileData = load(fullfile(folder,matchFiles{fileInd}));
    allClassOut{fileInd} = currFileData.classifierOut;
    allClassOutBreakCorr{fileInd} = currFileData.classifierOutBreakCorr;
end
...
if showErrorBars
    ...
    meanGuessBreakCorr = nan(length(matchFiles),13);
    for mouseInd = 1:length(matchFiles)
        for class = 1:length(testClassVals)
            ...
            % break corr 
            matchClassBreakCorr = ...
                allClassOutBreakCorr{mouseInd}(1).testClass == testClassVals(class);
            meanGuessBreakCorr(mouseInd,class) = nanmean(...
                allClassOutBreakCorr{mouseInd}(1).guess(matchClassBreakCorr));
        end
    end
    ...
    meanBreakCorrAllMice = nanmean(meanGuessBreakCorr);
    semBreakCorrAllMice = calcSEM(meanGuessBreakCorr);
    ...
    %break corr
    handles.errBreakCorr = errorbar(testClassVals,meanBreakCorrAllMice,semBreakCorrAllMice);
    handles.errBreakCorr.Marker = 'o';
    handles.errBreakCorr.MarkerFaceColor = handles.errBreakCorr.MarkerEdgeColor;
    handles.errBreakCorr.LineStyle = 'none';
    handles.errBreakCorr.MarkerSize = 3;
